    ...
    def check_timeout(self, is_profiling=False):
        ...
        if self.last_connected_time is None:
            ...
            if time.time() - self.last_switched_time < MAX_ALLOWED_DISCONNCTED_TIME:
                self.logger.info(
                    f"[{self.identity}] not connected, but has not been enough time since the last switch"
                )
                return False
            else:
                self.logger.warn(
                    f"[{self.identity}] not connected, timeout since the last switch"
                )
                return True
        else:
            # connected but last profile is too long ago
            self.logger.info(
                f"[{self.identity}] last connected time is {self.last_connected_time}, current time is {time.time()}, diff is {time.time() - self.last_connected_time}"
            )
...
class SGC_Policy_Scheduler(rclpy.node.Node):
    def __init__(self):
        ...
        self.max_num_waiting_profiles = self.get_parameter(
            "max_num_waiting_profiles"
        ).value
        ...
        self.current_active_service_machine = (
            self._get_service_machine_name_from_state_assignment()
        )

        # subscribe to the profile topic from other machines (if any)
        self.status_topic = self.create_subscription(
            Profile, "fogros_sgc/profile", self.profile_topic_callback, 10
        )

        self.optimize_profiling_server = self.create_service(
            SgcProfiling,
            "sgc_profiling_optimizer",
            self.sgc_profiling_optimizer_callback,
        )
        self.ls_active_profile_server = self.create_service(
            SgcProfiling, "sgc_profiling_ls", self.ls_active_profile_callback
        )
        self.get_machine_with_better_profile_server = self.create_service(
            SgcProfiling,
            "sgc_profiling_get_best",
            self.get_machine_with_better_profile_callback,
        )
        self.parallel_profiling_server = self.create_service(
            SgcProfiling, "sgc_parallel_profiling", self.sgc_parallel_profiling_callback
        )
        self.get_best_parallel_profiling_server = self.create_service(
            SgcProfiling,
            "sgc_parallel_profiling_get_best",
            self.sgc_parallel_profiling_best_callback,
        )

        # assignment service client to sgc_node
        self.assignment_service_client = self.create_client(
            SgcAssignment, "sgc_assignment_service"
        )

        self.max_latency_bound = (
            float(self.config["time_bound"]["max_latency"])
            if "max_latency" in self.config["time_bound"]
            else -1
        )
        self.mean_latency_bound = (
            float(self.config["time_bound"]["mean_latency"])
            if "mean_latency" in self.config["time_bound"]
            else -1
        )
        self.median_latency_bound = (
            float(self.config["time_bound"]["median_latency"])
            if "median_latency" in self.config["time_bound"]
            else -1
        )
        self.min_latency_bound = (
            float(self.config["time_bound"]["min_latency"])
            if "min_latency" in self.config["time_bound"]
            else -1
        )
        self.std_latency_bound = (
            float(self.config["time_bound"]["std_latency"])
            if "std_latency" in self.config["time_bound"]
            else -1
        )
        self.smart_bound = (
            float(self.config["time_bound"]["smart_latency"])
            if "smart_latency" in self.config["time_bound"]
            else -1
        )
    ...
    def sgc_parallel_profiling_best_callback(self, request, response):
        ...
        if self.is_parallel_profiling_ready and self.is_doing_parallel_profiling:
            ...
            if machine_with_better_profile is not None:
                ...
                self.logger.warn(
                    f"PARALLEL PROFILING CHOSE: {machine_with_better_profile}"
                )
            else:
    ...
    def profile_topic_callback(self, profile_update):
        if self.is_doing_parallel_profiling:
            self.service_dict[profile_update.identity.data].update(
                profile_update, self.is_doing_parallel_profiling
            )
            ...
            if self.service_dict[
                profile_update.identity.data
            ].has_run_out_of_skipped_profiles():
                self.logger.warn(
                    f"received a good, after trash, profile from {profile_update.identity.data}"
                )
                self._handle_profile_message_at_profiling(profile_update)

                num_servers = len(
                    self._get_nonrobot_machine_list_from_state_assignment()
                )

                if (
                    len(list(self.active_profiling_result.keys())) == num_servers
                    or time.time() - self.t0 > 0.05
                ):
        ...
        if profile_update.identity.data != current_service_machine:
            self.logger.info(
                f"received a profile from {profile_update.identity.data}, but the current active service machine is {current_service_machine}"
            )
            if self.service_dict[current_service_machine].check_timeout(
                self.is_doing_profiling
            ):
                self._handle_timeout()
            return

        self.service_dict[profile_update.identity.data].update(
            profile_update, self.is_doing_profiling
        )
        if self.service_dict[current_service_machine].check_timeout(
            self.is_doing_profiling
        ):
        ...
        self.logger.info(
            f"received a profile from {profile_update.identity.data}, and the current active service machine is {current_service_machine}"
        )

        if self.is_doing_profiling:
            if self.service_dict[
                current_service_machine
            ].has_run_out_of_skipped_profiles():
                self.logger.info(
                    f"received a profile from {profile_update.identity.data}, and the current active service machine is {current_service_machine}, and the scheduler is doing profiling"
                )
                self._handle_profile_message_at_profiling(profile_update)
        else:
    ...
    def _handle_timeout(self):
        if self.is_doing_profiling:
            self.logger.error(
                f"timeout detected at profiling, switch to the next unprofiled machine"
            )
            self.active_profiling_disconnected_list.append(
                self._get_service_machine_name_from_state_assignment()
            )
            self._switch_to_next_unprofiled_machine()
        else:
            self.logger.error(
                f"timeout detected at running, switch to the next machine with better profile"
            )
            if (
                self._get_service_machine_name_from_state_assignment()
                in self.active_profiling_result
            ):
                ...
                self.logger.error(
                    f"remove {self._get_service_machine_name_from_state_assignment()} from the already-profiled list"
                )
                del self.active_profiling_result[
                    self._get_service_machine_name_from_state_assignment()
                ]
    ...
    def _handle_profile_message_at_profiling(self, profile_update):
        if self.is_doing_parallel_profiling:
            self.logger.info(
                f"[profile-{profile_update.identity.data}] received an update from {profile_update}"
            )
            self.active_profiling_result[profile_update.identity.data] = profile_update

            return
        self.logger.info(
            f"[profile-{self._get_service_machine_name_from_state_assignment()}] received an update from {profile_update}"
        )
        self.active_profiling_result[
            self._get_service_machine_name_from_state_assignment()
        ] = profile_update
        self._switch_to_next_unprofiled_machine()
    ...
    def _handle_profile_message_at_running(self, profile_update):
        self.logger.info(
            f"[{self._get_service_machine_name_from_state_assignment()}]received an update from {profile_update}"
        )

        if not self._check_latency_bound(profile_update):
            self.logger.error(
                f"latency {profile_update.median_latency} does not fulfill the bound!!!!!"
            )
            if self.automatic_switching:
                ...
                if better_profile_machine is not None:
                    self.logger.info(
                        f"switching to {better_profile_machine} because it has better profile"
                    )
                    self._switch_to_machine(better_profile_machine)
            else:
    ...
    def _check_latency_bound(self, profile):
        ...
        if profile.median_latency == -1:
            self.logger.info(
                f"median latency is -1, no latency data is collected this window"
            )
            return False
        # smart
        if self.smart_bound != -1 and profile.max_kmeans_latency > self.smart_bound:
            self.logger.info(
                f"jenks latency {profile.max_kmeans_latency} is larger than the bound {self.smart_bound}"
            )
            return False
        # max
        if (
            self.max_latency_bound != -1
            and profile.max_latency > self.max_latency_bound
        ):
            self.logger.info(
                f"max latency {profile.max_latency} is larger than the bound {self.max_latency_bound}"
            )
            return False
        # mean
        if (
            self.mean_latency_bound != -1
            and profile.mean_latency > self.mean_latency_bound
        ):
            self.logger.info(
                f"mean latency {profile.mean_latency} is larger than the bound {self.mean_latency_bound}"
            )
            return False
        # median
        if (
            self.median_latency_bound != -1
            and profile.median_latency > self.median_latency_bound
        ):
            self.logger.info(
                f"median latency {profile.median_latency} is larger than the bound {self.median_latency_bound}"
            )
            return False
        # min
        if (
            self.min_latency_bound != -1
            and profile.min_latency < self.min_latency_bound
        ):
            self.logger.info(
                f"min latency {profile.min_latency} is smaller than the bound {self.min_latency_bound}"
            )
            # TODO: here need to switch to a worse compute machine
            return False
        # std
        if (
            self.std_latency_bound != -1
            and profile.std_latency > self.std_latency_bound
        ):
            self.logger.info(
                f"std latency {profile.std_latency} is larger than the bound {self.std_latency_bound}"
            )
            return False
        return True
    ...
    def _get_list_of_machine_not_profiled(self):
        ...
        profiled_machines = (
            list(self.active_profiling_result.keys())
            + [self._get_robot_machine_name_from_state_assignment()]
            + self.active_profiling_disconnected_list
        )
        self.logger.info(
            f"[Profile status] All machines {all_machines}, profiled machines {profiled_machines}, yet from profile machine {list(set(all_machines) - set(profiled_machines))}"
        )
    ...
    def get_a_machine_with_better_profile(self, exclude_list=[]):
        ...
        if len(all_passed_machines) == 0:
            ...
            if len(connected_machines) == 0:
                self.logger.error(
                    f"no machine in profile is marked as connected, do nothing"
                )
                ...
                self.logger.error(
                    f"some machines are connected, switch to some connected one"
                )
                sorted_machines = sorted(
                    connected_machines,
                    key=lambda x: self.active_profiling_result[x].max_latency,
                )
            ...
            self.logger.info(
                f"machine {passed_machine} is passed the latency bound, switch to one of them"
            )
            return passed_machine

    def _get_machine_standby(self):
        standby_machine = random.choice(
            self._get_standby_machine_list_from_state_assignment()
        )
        self.logger.info(
            f"switch to {standby_machine} because it is the only standby machine"
        )
    ...
    def _get_machine_with_best_network(self):
        ...
        for machine in self.machine_profile_dict.keys():
            network_info = ping_host(
                self.service_dict[machine].last_profile.ip_addr.data
            )
    ...
    def _switch_to_next_unprofiled_machine(self):
        ...
        if len(unprofiled_machines) == 0:
            ...
            unprofiled_machines_other_than_current_service = [
                x
                for x in unprofiled_machines
                if x != self._get_service_machine_name_from_state_assignment()
            ]
            if len(unprofiled_machines_other_than_current_service) == 0:
                ...
                self._switch_to_machine(
                    random.choice(unprofiled_machines_other_than_current_service)
                )

    def _switch_to_machine(self, machine_new, force=False):
        previous_service_machine = (
            self._get_service_machine_name_from_state_assignment()
        )
        if not force and machine_new == previous_service_machine:
            self.logger.warn(
                f"{machine_new} is the same as the current machine, not switching"
            )
            return
    ...
    def _switch_to_all(self, force=False):
        ...
        for machine in self._get_standby_machine_list_from_state_assignment():
            if not force and machine == previous_service_machine:
                self.logger.warn(
                    f"{machine} is the same as the current machine, not switching"
                )
                continue
